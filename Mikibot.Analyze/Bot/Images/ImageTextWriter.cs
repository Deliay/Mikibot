...
public static class ImageTextWriter
{
    ...
    static ImageTextWriter()
    {
        ...
        FontCanvasWidth = (int)Math.Round(1.0 * (totalSize.Width + totalSize.X), MidpointRounding.AwayFromZero);
        FontCanvasLineHeight = (int)Math.Round(1.5 * (totalSize.Height + totalSize.Y), MidpointRounding.AwayFromZero);
    }
