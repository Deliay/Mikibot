    ...
    : MiraiGroupMessageProcessor<LlmChatbot>(miraiService, logger)
{
    ...
    private const string BasicPrompt =
        ...
        "请在上下文中精炼出最多3个话题，并给出你的回复，字数可以从1-25字不等，请灵活的安排你要回复的内容，需要长的时候长，需要短的时候短。" +
        "回复文本不用太正式，回复内容也尽量口语化。 以JSON数组的形式输出，以JSON数组的形式输出，话题中有地域黑的得分一律为100。" +
        "JSON格式为：[{ \"topic\": \"你推测的关于这个的话题\", \"reply\": \"你回复的消息\", \"score\": 角色设定对该话题的分数(0-100)  },...]";
