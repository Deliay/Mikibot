...
namespace Mikibot.Analyze.Notification
{
    ...
    public class DailyFollowerStatisticService(IMiraiService mirai, ILogger<DailyFollowerStatisticService> logger)
    {
        ...
        private async Task DailyReportAll(CancellationToken token)
        {
            ...
            {
                await DailyReport(subscription.UserId, token);
            }
        }
        
        private async Task DailyReport(string userId, CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
        ...
        public async Task WeeklyReport(string userId, int targetCount, CancellationToken token)
        {
            ...
            await Mirai.SendMessageToAllGroup(token, new PlainMessage(msg));
        }
        ...
        public async Task WeeklyReportSchedule(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var subscription in subscriptions)
                    {
                        await WeeklyReport(subscription.UserId, subscription.TargetFansCount, token);
                    }
